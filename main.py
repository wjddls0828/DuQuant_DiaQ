...
def main():
    ...
    for fullname, layer in quantlinear_layers.items():
        # 'model.layers.0.self_attn.k_proj' 에서 'self_attn.k_proj' 부분만 추출
        m = re.search(r'model\.layers\.\d+\.(.+)', fullname)
        if not m:
            continue
        layer_type = m.group(1)  # e.g. 'self_attn.k_proj'
        
        for key, values in layer.stats.items():
            # stats[key] 는 보통 리스트이므로 extend
            stats_by_type[layer_type][key].extend(values)
    mean_stats = {}
    for layer_type, key_dict in stats_by_type.items():
        mean_stats[layer_type] = {
            key: float(np.mean(vals))  # numpy로 평균 계산
