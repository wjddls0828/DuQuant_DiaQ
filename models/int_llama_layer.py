...
class QuantLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        query_states, query_orig = self.qkt_matmul.quant_x1(query_states)
        x1len= torch.norm(query_orig, p=2, dim=-1, keepdim=True)
        ...
        key_states, key_orig = self.qkt_matmul.quant_x2(key_states)
        x2len= torch.norm(key_orig, p=2, dim=-1, keepdim=True)
        ...
        attn_weights= self.pv_matmul.quant_x1(attn_weights, True)

        value_states, value_orig = self.pv_matmul.quant_x2(value_states)
        x2len= torch.norm(value_orig, p=2, dim=-1, keepdim=True)
